...
import { Camera, User, Mail, Phone, MapPin, Calendar, Briefcase, Save, AlertCircle, CheckCircle, Shield, Clock, Edit } from 'lucide-react';
...
function ProfilePage() {
  ...
  const loadProfile = async () => {
    try {
      setInitialLoad(true);
      console.log('🔄 FRONTEND - Iniciando carga de perfil...');
      console.log('👤 FRONTEND - Usuario actual:', user?.id, user?.username);
      ...
      if (response.ok) {
        ...
        console.log('📥 FRONTEND - Datos recibidos del servidor:', data);
        console.log('📋 FRONTEND - Campos específicos:');
        console.log('  - telefono:', data.telefono, typeof data.telefono);
        console.log('  - direccion:', data.direccion, typeof data.direccion);
        console.log('  - especialidad:', data.especialidad, typeof data.especialidad);
        console.log('  - biografia:', data.biografia, typeof data.biografia);
        ...
        console.log('💾 FRONTEND - Datos que se van a guardar en state:', newProfileData);
        console.log('📋 FRONTEND - Campos específicos del nuevo state:');
        console.log('  - telefono:', newProfileData.telefono);
        console.log('  - direccion:', newProfileData.direccion);
        console.log('  - especialidad:', newProfileData.especialidad);
        console.log('  - biografia:', newProfileData.biografia);
        ...
        // Verificar que el state se actualizó correctamente
        setTimeout(() => {
          console.log('✅ FRONTEND - State después de actualizar (verificación):', {
            telefono: newProfileData.telefono,
            direccion: newProfileData.direccion,
            especialidad: newProfileData.especialidad,
            biografia: newProfileData.biografia
          });
        }, 100);
        
      } else {
        console.log('❌ FRONTEND - Error en la respuesta:', response.status);
        const errorText = await response.text();
        console.log('❌ FRONTEND - Error del servidor:', errorText);
      }
    } catch (error) {
      console.error('💥 FRONTEND - Error al cargar perfil:', error);
      setErrors({ general: 'Error al cargar el perfil: ' + error.message });
    } finally {
  ...
  const handleInputChange = (e) => {
    ...
    console.log('📝 FRONTEND - Campo cambiado:', name, 'nuevo valor:', value);
    setProfileData(prev => {
      const updated = { ...prev, [name]: value };
      console.log('📊 FRONTEND - Estado actualizado:', updated);
      return updated;
    });
  ...
  const handleDayChange = (day) => {
    ...
    console.log('🗓️ FRONTEND - Días actualizados:', updatedDays);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('📤 FRONTEND - Enviando datos del formulario:', profileData);
      
      const formData = new FormData();
      
      // Agregar todos los campos de texto
      Object.keys(profileData).forEach(key => {
        if (key !== 'foto' && key !== 'username' && key !== 'email' && profileData[key] !== null && profileData[key] !== undefined) {
          if (key === 'diasTrabajo') {
            // Enviar array de días como JSON string
            formData.append(key, JSON.stringify(profileData[key]));
            console.log('📤 FRONTEND - Enviando diasTrabajo:', JSON.stringify(profileData[key]));
          } else {
            formData.append(key, profileData[key]);
            console.log(`📤 FRONTEND - Enviando ${key}:`, profileData[key]);
          }
        }
      });
      ...
      if (profileData.foto && typeof profileData.foto === 'object') {
        ...
        console.log('📷 FRONTEND - Enviando foto:', profileData.foto.name);
      }
      ...
      if (response.ok) {
        ...
        setProfileData(prev => ({
          ...
          fechaNacimiento: result.fechaNacimiento ? result.fechaNacimiento.split('T')[0] : '',
          especialidad: result.especialidad || '',
          numeroLicencia: result.numeroLicencia || '',
          biografia: result.biografia || '',
          diasTrabajo: result.diasTrabajo || [],
          horarioInicio: result.horarioInicio || '8:00 AM',
          horarioFin: result.horarioFin || '5:00 PM'
        }));

        // Actualizar foto si se envió una nueva
        if (result.foto) {
          setPreview(`http://localhost:3000/uploads/profiles/${result.foto}`);
        }
      } else {
      ...
      console.error('💥 FRONTEND - Error al actualizar perfil:', error);
  ...
  return (
    <div className="min-h-screen bg-pastel-mint p-6">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-pastel-secondary">
                {editMode ? 'Editando información personal y profesional' : 'Información personal y profesional'}
              </p>
            </div>
        ...
        {/* Debug info - temporal */}
        <div className="mb-6 card-pastel bg-pastel-yellow p-4 rounded-lg">
          <h4 className="font-semibold text-pastel-primary mb-2">🔍 Debug Info (temporal):</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            <div>
              <p className="font-semibold">Estado actual:</p>
              <pre className="text-pastel-secondary overflow-auto bg-white p-2 rounded mt-1">
{JSON.stringify({
  telefono: profileData.telefono,
  direccion: profileData.direccion,
  especialidad: profileData.especialidad,
  biografia: profileData.biografia,
  diasTrabajo: profileData.diasTrabajo
}, null, 2)}
              </pre>
            </div>
            <div>
              <p className="font-semibold">Info adicional:</p>
              <div className="bg-white p-2 rounded mt-1 text-pastel-secondary">
                <p>Edit Mode: {editMode ? 'SÍ' : 'NO'}</p>
                <p>User ID: {user?.id}</p>
                <p>Datos cargados: {initialLoad ? 'Cargando...' : 'Completado'}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              {profileData.diasTrabajo && profileData.diasTrabajo.length > 0 ? (
                <div className="space-y-3">
                  <div className="bg-white p-4 rounded-lg">
                    <p className="text-sm font-medium text-pastel-primary mb-2">Días laborables:</p>
                    <div className="flex flex-wrap gap-2">
                      {profileData.diasTrabajo.map(dia => (
                        <span key={dia} className="bg-pastel-green px-3 py-1 rounded-full text-xs font-medium text-green-700">
                          {dia}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="bg-white p-4 rounded-lg">
                    <p className="text-sm font-medium text-pastel-primary mb-2">Horario:</p>
                    <p className="text-pastel-secondary">
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-pastel-muted text-sm">No hay horario configurado</p>
                </div>
              )}
            </div>
          </div>
                  ...
                  {user?.role === 'odontologo' ? (
                    ...
                    <div className="card-pastel p-4 bg-pastel-purple text-center">
                      ...
                      <p className="text-purple-700 text-sm">
                        Como administrador del sistema, tu especialidad es la gestión completa
                      </p>
                    </div>
                  )}
                </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                          {diasSemana.map(day => (
                            <label key={day} className="flex items-center space-x-2 cursor-pointer">
                              ...
                              />
                              <span className="text-sm text-pastel-primary">{day}</span>
                            </label>
                          ))}
                        </div>
                      </div>
        ...
        <div className="mt-8 card-pastel p-6 bg-pastel-green">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm text-pastel-secondary">
            <ul className="space-y-2">
              <li>• <strong>Modo de edición:</strong> Haz clic en "Editar Perfil" para modificar información</li>
              <li>• <strong>Foto profesional:</strong> Una buena foto aumenta la confianza de los pacientes</li>
              <li>• <strong>Horarios de trabajo:</strong> Configura tus días laborables para mejor organización</li>
            </ul>
            <ul className="space-y-2">
              <li>• <strong>Información pública:</strong> Tu perfil es visible en la página principal</li>
              <li>• <strong>Privacidad:</strong> Tu información está protegida y es confidencial</li>
              <li>• <strong>Actualización:</strong> Puedes modificar tu información cuando sea necesario</li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="card-pastel p-4 bg-pastel-purple text-center">
            ...
            <p className="text-lg font-bold text-purple-700">
              {Math.round((Object.values(profileData).filter(val => 
